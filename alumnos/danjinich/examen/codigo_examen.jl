...
function best_cameras(s, df; tol::Float64=1e-4, maxit::Int=10000)
	x0=rand_geo_array(minimum(df["lat"]),maximum(df["lat"]),minimum(df["long"]),maximum(df["long"]), s)
	f(x)=costo(x,df)
	x=line_search_newton_modification(f, x0; tol=tol, maxit=maxit)
	return cat(x[1:s],x[s+1:2*s], dims=2)
end
...
function distance(x0, lat, lon)
	n=floor(Int,length(x0)/2)
	res=Array{Float64}(undef, n)
	for i in 1:n
		res[n]=(x0[i]-lat)^2+(x0[i+n]-lon)^2
	end
	return minimum(res)
end
...
function costo(x0, df)
	...
	Threads.@threads for i in 1:n
		res[i]=distance(x0,df["lat"][i],df["long"][i])
	end
	return sum(res)
end
...
df = CSV.read("/home/danjf/Documents/crime_data.csv");
s=8000
best_cameras(s,df;maxit=10000, tol=1e-4)
